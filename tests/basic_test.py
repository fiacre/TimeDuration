...
class basic_test(unittest.TestCase):
    def testInstance (self) :
        a = td()
        self.assertEqual(isinstance(a, td), True)
    ...
    def testNormalized_3 (self) :
        ...
        for i in range(len(time_array)) :
            ...
            for i in range(len(td_array)) :
                if i == 4 :
                    self.assertEqual( str(td_array[i]), str(td("120:01:25.0").normalized()) ) 
